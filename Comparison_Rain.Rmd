...
names(df_qc)[names(df_qc) == "Total Rain (mm)"] <- "Total_Rain_mm"
df_qc <- df_qc %>% dplyr::filter(.,Year<=1992) %>% mutate(.,city=1)
...
names(df_mtl)[names(df_mtl) == "Total Rain (mm)"] <- "Total_Rain_mm"
...
names(df_tr)[names(df_tr) == "Total Rain (mm)"] <- "Total_Rain_mm"
...
names(df_vc)[names(df_vc) == "Total Rain (mm)"] <- "Total_Rain_mm"
...
df <- rbind(df,df_vc)
df <- rbind(df,df_tr)
df <- drop_na(df,Total_Rain_mm) %>% group_by(Date_Time,city) %>% top_n(1, Total_Rain_mm)
```
...
```{r}
df_bm <- df %>% group_by(Year,city)  %>% summarise(.,max = max(Total_Rain_mm)) %>% drop_na(.)
```
...
```{r}
#Function that computes return levels and 95% confidence interval for different return levels
return_levels_bm_shape0 <- function(df_bm,levels){
...
```{r}
levels <- append(c(2:10),c(20,25,50,75,100))
df_bm_shape0 <- return_levels_bm_shape0(df_bm,levels)
df_bm_shape0
```

```{r}
ggplot(df_bm_fin, aes(x=p, y= Return_level,col=factor(city, levels=c(1,2,3,4), labels=c("Quebec", "Montreal", "Toronto", "Vancouver")))) + geom_point(size=2) + xlab("Number of Years") + ylab("Return Level") + geom_ribbon(data=filter(df_bm_fin,p>0),aes(ymin=(Return_level - Confidence_int),ymax=(Return_level + Confidence_int),fill=factor(city, levels=c(1,2,3,4), labels=c("Quebec", "Montreal", "Toronto", "Vancouver"))),alpha=0.3) + guides(fill = FALSE) + ggtitle("Return Levels for Maxima of Temperature in July\nfor Cities in Canada (Block Maxima shape=Free)") + theme(plot.title = element_text(size=18, face="bold", margin = margin(10, 0, 10, 0))) + theme(axis.text.x=element_text(angle=0, size=12, vjust=0.5)) + theme(axis.text.y=element_text(angle=0, size=12, vjust=0.5)) + theme(
...
```{r}
ggplot(df_bm_shape0, aes(x=p, y= Return_level,col=factor(city, levels=c(1,2,3,4), labels=c("Quebec", "Montreal", "Toronto", "Vancouver")))) + geom_point(size=2) + xlab("Number of Years") + ylab("Return Level") + geom_ribbon(data=filter(df_bm_shape0,p>0),aes(ymin=(Return_level - Confidence_int),ymax=(Return_level + Confidence_int),fill=factor(city, levels=c(1,2,3,4), labels=c("Quebec", "Montreal", "Toronto", "Vancouver"))),alpha=0.3) + guides(fill = FALSE) + ggtitle("Return Levels for Maxima of Temperature in July\nfor Cities in Canada (Block Maxima shape=0)") + theme(plot.title = element_text(size=18, face="bold", margin = margin(10, 0, 10, 0))) + theme(axis.text.x=element_text(angle=0, size=12, vjust=0.5)) + theme(axis.text.y=element_text(angle=0, size=12, vjust=0.5)) + theme(
  axis.title.x = element_text(size = 14,color="black", vjust=-0.35),
      axis.title.y = element_text(size = 14,color="black" , vjust=0.35)   
          ) + scale_color_discrete(name="") + scale_color_manual(values=c("#FFCC00", "#FF6600","#990000" ,"009900"),name="") + scale_fill_manual(values=c(NA, NA,NA ,NA)) 
```
...
```{r}
#Because of the rejection of the null hypothesis that xi=0 is an accurate model for Quebec, new plot
df_temp_bm <- df_bm_fin %>% dplyr::filter(.,city==1)
df_temp_bm1 <- df_bm_shape0 %>% dplyr::filter(.,city>=2)
df_new_comp <- rbind(df_temp_bm,df_temp_bm1)
ggplot(df_new_comp, aes(x=p, y= Return_level,col=factor(city, levels=c(1,2,3,4), labels=c("Quebec", "Montreal", "Toronto", "Vancouver")))) + geom_point(size=2) + xlab("Number of Years") + ylab("Return Level") + geom_ribbon(data=filter(df_new_comp,p>0),aes(ymin=(Return_level - Confidence_int),ymax=(Return_level + Confidence_int),fill=factor(city, levels=c(1,2,3,4), labels=c("Quebec", "Montreal", "Toronto", "Vancouver"))),alpha=0.3) + guides(fill = FALSE) + ggtitle("Return Levels for Maxima of Temperature in July\nfor Cities in Canada (Block Maxima shape=0)") + theme(plot.title = element_text(size=18, face="bold", margin = margin(10, 0, 10, 0))) + theme(axis.text.x=element_text(angle=0, size=12, vjust=0.5)) + theme(axis.text.y=element_text(angle=0, size=12, vjust=0.5)) + theme(
  axis.title.x = element_text(size = 14,color="black", vjust=-0.35),
      axis.title.y = element_text(size = 14,color="black" , vjust=0.35)   
          ) + scale_color_discrete(name="") + scale_color_manual(values=c("#FFCC00", "#FF6600","#990000" ,"009900"),name="") + scale_fill_manual(values=c(NA, NA,NA ,NA)) 
```
...
FOR THIS SECTION ON R-LARGEST ORDER STATISTIC, RUN THE FILES Montreal_Rain, Quebec_Rain, Toronto_Rain and Vancouver_Rain prior to this. Some variables need to be saved in the local environment
...
```{r}
df_r_2_mtl <- df_r_2_mtl %>% mutate(city=2)
df_r_4_qc <- df_r_4_qc %>% mutate(city=1)
df_r_1_tr <- df_r_1_tr %>% mutate(city=3)
df_r_1_vc <- df_r_1_vc %>% mutate(city=4)
df_r_comp <- rbind(df_r_2_mtl,df_r_4_qc,df_r_1_tr,df_r_1_vc)
```

```{r}
ggplot(df_r_comp, aes(x=p, y= Return_level,col=factor(city, levels=c(1,2,3,4), labels=c("Quebec", "Montreal", "Toronto", "Vancouver")))) + geom_point(size=2) + xlab("Number of Years") + ylab("Return Level") + geom_ribbon(data=filter(df_r_comp,p>0),aes(ymin=(Return_level - Confidence_int),ymax=(Return_level + Confidence_int),fill=factor(city, levels=c(1,2,3,4), labels=c("Quebec", "Montreal", "Toronto", "Vancouver"))),alpha=0.3) + guides(fill = FALSE) + ggtitle("Return Levels for Maxima of Temperature in July\nfor Cities in Canada (Block Maxima shape=0)") + theme(plot.title = element_text(size=18, face="bold", margin = margin(10, 0, 10, 0))) + theme(axis.text.x=element_text(angle=0, size=12, vjust=0.5)) + theme(axis.text.y=element_text(angle=0, size=12, vjust=0.5)) + theme(
  axis.title.x = element_text(size = 14,color="black", vjust=-0.35),
      axis.title.y = element_text(size = 14,color="black" , vjust=0.35)   
          ) + scale_color_discrete(name="") + scale_color_manual(values=c("#FFCC00", "#FF6600","#990000" ,"009900"),name="") + scale_fill_manual(values=c(NA, NA,NA ,NA)) 
```
...
ggplot(df_final, aes(x=p, y= Return_level,col=factor(city, levels=c(1,2,3,4,5,6,7,8), labels=c("Quebec BM", "Montreal BM", "Toronto BM", "Vancouver BM","Quebec T", "Montreal T", "Toronto T", "Vancouver T")))) + geom_point() + xlab("Number of Years") + ylab("Return Level") + geom_ribbon(data=filter(df_final,p>0),aes(ymin=(Return_level - Confidence_int),ymax=(Return_level + Confidence_int),fill=factor(city, levels=c(1,2,3,4,5,6,7,8), labels=c("Quebec BM", "Montreal BM", "Toronto BM", "Vancouver BM","Quebec T", "Montreal T", "Toronto T", "Vancouver T"))),alpha=0.3) + guides(fill = FALSE) + ggtitle("Return Levels for Maxima of Temperature in July\nfor Cities in Canada") + theme(plot.title = element_text(size=18, face="bold", margin = margin(10, 0, 10, 0))) + theme(axis.text.x=element_text(angle=0, size=12, vjust=0.5)) + theme(axis.text.y=element_text(angle=0, size=12, vjust=0.5)) + theme(
  axis.title.x = element_text(size = 14,color="black", vjust=-0.35),
      axis.title.y = element_text(size = 14,color="black" , vjust=0.35)   
          ) + scale_color_discrete(name="") + scale_fill_manual(values=c(NA,NA,NA,NA,NA,NA,NA,NA)) + scale_color_manual(values=c("#FFCC00", "#FF6600","#990000" ,"009900","#FFCC00", "#FF6600","#990000" ,"009900"),name="")
```