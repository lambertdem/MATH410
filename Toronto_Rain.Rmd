...
```{r, warning=FALSE}
df <- read_csv("Datasets/QC_1943_2020.csv") #QC_1943_2020.csv #VC_1937_2020.csv #TR_1937_2020.csv
...
```{r}
ggplot(df, aes(x=Date_Time, y= Total_Rain_mm)) + geom_point() + xlab("Date") + ylab("Rain (mm)") + ggtitle("Montreal Rain Precipitations (mm)") + geom_abline(intercept = mod$coefficients[1], slope = mod$coefficients[2], color="red") + theme(plot.title = element_text(size=18, face="bold", 
...
return_levels_bm <- function(model,levels){
  ...
  for(i in 1:length(levels)){
    ...
    grad_Zp <- c(1, a, b)
    var_Zp <- t(grad_Zp) %*% model1$var.cov %*% grad_Zp
...
G_inv_zi <- mu - (sig/xi)*(1-(-log(index/(length(z)+1)))^(-1*xi))
df_quantile_plot <- data.frame(cbind(index,z,G_inv_zi))
ggplot(df_quantile_plot, aes(x=z, y= G_inv_zi)) + geom_point() + xlab("Ordered block maxima") + ylab("Estimated inverse function G") + ggtitle("Quantile Plot for the r-Largest Order Statistics (r=4)") + geom_abline(intercept = 0, slope = 1, color="firebrick") + theme(plot.title = element_text(size=18, face="bold", 
    margin = margin(10, 0, 10, 0))) + theme(axis.text.x=element_text(angle=0, size=12, vjust=0.5)) + theme(axis.text.y=element_text(angle=0, size=12, vjust=0.5)) + theme(
      axis.title.x = element_text(size = 14,color="black", vjust=-0.35),
      axis.title.y = element_text(size = 14,color="black" , vjust=0.35)   
          )
