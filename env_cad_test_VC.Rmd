...
```{r, warning=FALSE}
df <- read_csv("VC_1937_2020.csv") #QC_1943_2020.csv
...
```{r}
r_1 <- rlarg.fit(rank_k[,-1],r=2)
```

```{r}
r_1 <- rlarg.fit(rank_k[,-1],r=5)
```

```{r}
r_1 <- rlarg.fit(rank_k[,-1],r=10)
```
...
mrlplot(obs, tlim = c(26,30), nt = 20,
  ...
  legend.loc = "bottomleft", try.thresh = 28, main = "Mean Residual Life Plot", xlab = "Threshold u",
  ylab = "Mean Excess")
```
