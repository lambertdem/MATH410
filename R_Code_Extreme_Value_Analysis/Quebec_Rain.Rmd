...
```{r}
df <- drop_na(df,Total_Rain_mm) %>% group_by(Date_Time) %>% top_n(1, Total_Rain_mm) %>% dplyr::filter(.,Year<=1992)
df
```
...
df_r_fin_comp <- df_r %>% dplyr::filter(.,r==4) %>% dplyr::select(.,p,Return_level,Confidence_int) %>% dplyr::mutate(.,mod = "r=4")
```

```{r}
#SAVE BEST r-LARGEST ORDER STATISTIC MODEL FOR COMPARISON IN Comparison_Rain.rmd
r_4_qc <- r_4
df_r_4_qc <- df_r_fin_comp
```
#########################################################################################################################################################################################
#########################################################################################################################################################################################
#########################################################################################################################################################################################
