...
```{r}
levels <- append(c(2:10),c(20,50,100))
df <- return_levels_bm(model1,levels)
df
```
...
```{r}
#Function that computes return levels and 95% confidence interval for different return levels
return_levels_r <- function(model,levels){
  mu <- r_2$mle[1]
  sig <- r_2$mle[2]
  xi <- r_2$mle[3]
  ret_lev = list()
  ...
  for(i in 1:length(levels)){
    ...
    b <-  (sig / (xi^2)) * (1 - yp^(-1*xi)) - (sig / (xi)) * ( yp^(-1*xi)) * log(yp)
    grad_Zp <- c(1, a, b)
    var_Zp <- t(grad_Zp) %*% model$cov %*% grad_Zp
    conf <- 1.96*sqrt(var_Zp)
  ...
  b <-  (sig / (xi^2))
  grad_Zp <- c(1, a, b)
  var_Z_0 <- t(grad_Zp) %*% model$cov %*% grad_Zp
  conf_int[length(conf_int)+1] <- 1.96*sqrt(var_Z_0)
  df <- data.frame(cbind(c(unlist(levels),0),unlist(ret_lev),unlist(conf_int)))
  names(df) <- c("p","Return_level","Confidence_int")
  return (df)
...
```{r}
levels <- append(c(2:10),c(20,50,100))
df <- return_levels_r(r_2,levels)
df
```

```{r}
levels <- append(c(2:10),c(20,50,100))
df <- return_levels_r(r_3,levels)
df
```
...
return_levels_thresh <- function(obs,u,levels){
  ...
  for(j in 1:length(u)){
    ...
    for(i in 1:length(levels)){
      ...
      x_m <- u + (sig/xi)*((m*sampl_prop)^xi -1)
  ...
  names(df) <- c("u","p","Return_level","Confidence_int")
  return (df)
}
```
...
u_0 <- 29
levels <- append(c(2:10),c(20,50,100))
df <- return_levels_thresh(obs,u_0,levels)
df
```
